...
class SFTPS3Server extends EventEmitter {
  ...
  addPublicKey(key, username, ns) {
    ...
    console.log(util.format('Added public key for username %s, path %s', username, path));
  }
  ...
  removePublicKey(username) {
    _.remove(this.publicKeys, (p) => {
      console.log(util.format('Removed public key for username %s', username));
   ...
   */
  removeAllPublicKeys() {
    console.log(util.format('Removed all public keys'));
    this.publicKeys = [];
  }
    ...
    this.ssh = new ssh2.Server({
      ...
      .on('ready', () => {
        client.on('session', (accept, reject) => {
          var session = accept();
          console.log(util.format('logging on %s', pubKey.username));
          ...
          session.on('sftp', (accept, reject) => {
            ...
            sftpStream.on('OPEN', (reqid, filename, flags, attrs) => {
              console.log(util.format('SFTP %s', 'OPEN'));
              ...
              console.log(util.format('SFTP %s', 'READ'));
              ...
              console.log(util.format('SFTP %s', 'WRITE'));
              if (handle.length !== 4 || !openFiles[handle.readUInt32BE(0, true)])
                return sftpStream.status(reqid, STATUS_CODE.FAILURE);
              ...
              console.log(util.format('SFTP %s', 'OPENDIR'));
              ...
              console.log(util.format('SFTP %s', 'READDIR'));
              ...
              console.log(util.format('SFTP %s', 'REALPATH'));
              if(path === '.')
                path = '/';
              var p = path;
              p = p.replace('\\\\', '/');
              ...
              console.log(util.format('listing objects under %s (%s)', fullname, p));
              this.s3.listObjects({
                Prefix: fullname
              }, (err, data) => {
                if(err) {
                ...
                console.log(util.format('%d objects found', data.Contents.length));

                var realObj = _.find(data.Contents, (c) => c.Key === fullname || c.Key === (fullname + '/.dir'));

                if(realObj && realObj.Key.endsWith('.dir')) {
                  console.log(util.format('$s is a directory', realObj.Key));
                  realObj.IsDir = true;
                }

                if(!realObj && p === '/') {
                  console.log(util.format('listing empty root directory %s', p));
                  realObj = {
                    IsDir: true,
                ...
                if(!realObj) {
                  console.log(util.format('no objects found at %s', fullname));
                  return sftpStream.status(reqid, STATUS_CODE.NO_SUCH_FILE);
                }
              ...
              console.log(util.format('SFTP %s', 'CLOSE'));
              ...
              console.log(util.format('SFTP %s', 'REMOVE'));
              var fullname = this._mapKey(pubKey.path, path);
              ...
              console.log(util.format('SFTP %s', 'RMDIR'));
              var fullname = this._mapKey(pubKey.path, path);
              ...
              console.log(util.format('SFTP %s', 'MKDIR'));
              ...
              console.log(util.format('SFTP %s', 'RENAME'));
            ...
            function onStat(reqid, path) {
              console.log(util.format('SFTP %s', 'STAT/LSTAT'));
              var fullname = this._mapKey(pubKey.path, path);
