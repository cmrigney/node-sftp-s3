  ...
  listen(port, bindAddress, callback) {
    ...
    this.ssh = new ssh2.Server({
      ...
      client.on('authentication', (ctx) => {
        ...
        if(!pubKey || pubKey.username !== ctx.username) {
          return ctx.reject();
        }
            ...
            sftpStream.on('OPEN', (reqid, filename, flags, attrs) => {
              ...
              if(flags & OPEN_MODE.READ) {
                ...
                this.s3.createMultipartUpload({
                  ...
                  var handle = new Buffer(4);
                  openFiles[handleCount] = { flags: flags, filename: filename, fullname: fullname, uploadId: data.UploadId, partNumber: 1, multiparts: [] };
              ...
              let currentPartNumber = state.partNumber;

              this.s3.uploadPart({
                Key: state.fullname,
                PartNumber: String(state.partNumber++),
              ...
              sftpStream.name(reqid, state.listings.map((l) => {
                ...
                return {
                  filename: filename,
                  longname: `${l.IsDir ? 'd' : '-'}rw-rw-rw-    1 ${pubKey.path}  ${pubKey.path} ${l.Size} ${lastModified.format('MMM D')} ${moment().year() === lastModified.year() ? lastModified.format('HH:mm') : lastModified.format('YYYY')} ${_path.basename(filename)}`,
                  attrs: attrs
                };
              }));
              ...
              this.s3.listObjects({
                ...
                var name = [{
                  filename: p,
                  longname: `${realObj.IsDir ? 'd' : '-'}rw-rw-rw-    1 ${pubKey.path}  ${pubKey.path} ${realObj.Size} ${lastModified.format('MMM D')} ${moment().year() === lastModified.year() ? lastModified.format('HH:mm') : lastModified.format('YYYY')} ${_path.basename(p) || p}`
                }];
